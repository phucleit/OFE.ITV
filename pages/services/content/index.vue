<template>
  <div class="w-full flex flex-wrap mt-4 services-list">
      ...
      <template #content class="w-full">
        <div class="flex w-full justify-between items-center">
          <span class="flex-grow">
            <span>Danh sách dịch vụ Nội dung Marketing</span>
          </span>
    ...
    <span class="shadow-md p-3 h-full w-full flex bg-white rounded-lg">
      <div v-if="contents" class="flex w-full flex-wrap p-3">
        <ServicesContentList v-model="contents" class="w-full"></ServicesContentList>
      </div>
    </span>
    ...
    >
      <el-form>
        <el-form-item label="Tên gói nội dung (*)">
          <el-select v-model="model.plan" size="small" filterable placeholder="Chọn gói nội dung">
            <el-option
              v-for="item in contentPlans"
              :key="item._id"
              :label="item.name"
              :value="item._id"
            >
...
<script>
  export default {
    ...
    computed: {
      contents() {
        return JSON.parse(JSON.stringify(this.$store.state.content.model));
      },
