<template>
  <div class="w-full flex flex-wrap mt-4 services-list">
      ...
      <template #content class="w-full">
        <div class="flex w-full justify-between items-center">
          <span class="flex-grow">
            <span>Danh sách dịch vụ Quảng cáo Google</span>
          </span>
    ...
    <span class="shadow-md p-3 h-full w-full flex bg-white rounded-lg">
      <div v-if="googles" class="flex w-full flex-wrap p-3">
        <ServicesGoogleList v-model="googles" class="w-full"></ServicesGoogleList>
      </div>
    </span>
    ...
    >
      <el-form>
        <el-form-item label="Tên gói google (*)">
          <el-select v-model="model.plan" size="small" filterable placeholder="Chọn gói google">
            <el-option
              v-for="item in googlePlans"
              :key="item._id"
              :label="item.name"
              :value="item._id"
            >
...
<script>
  export default {
    ...
    computed: {
      googles() {
        return JSON.parse(JSON.stringify(this.$store.state.google.model));
      },
