<template>
  <div class="w-full flex flex-wrap">
    ...
    <div class="w-full mt-4 grid grid-cols-1 md:grid-cols-2 gap-3">
      ...
      <div class="flex-noshink">
        <div v-if="domains.data" class="flex w-full flex-wrap p-3">
          <section class="w-full bg-white shadow-md rounded-lg p-3">
            <span>Dịch vụ tên miền</span>
            <CustomerDomainService v-model="domains" class="w-full"></CustomerDomainService>
          </section>
        </div>
        <div v-if="hostings.data" class="flex w-full flex-wrap p-3">
          <section class="w-full bg-white shadow-md rounded-lg p-3">
...
<script>
  export default {
    ...
    methods: {
      ...
      async save() {
        try {
          await this.$store.dispatch('customer/update', { id: this.model._id, data: this.model});
          this.$message({
    ...
    async mounted() {
      let res = await this.$store.dispatch('customer/get', this.$route.params.id);
      this.model = res.data;

      await this.$store.dispatch('customer/getDomain', this.$route.params.id);
      await this.$store.dispatch('customer/getHosting', this.$route.params.id);
      await this.$store.dispatch('customer/getSsl', this.$route.params.id);
      await this.$store.dispatch('customer/getEmail', this.$route.params.id);
      await this.$store.dispatch('customer/getGoogle', this.$route.params.id);
      await this.$store.dispatch('customer/getFacebook', this.$route.params.id);
      await this.$store.dispatch('customer/getContent', this.$route.params.id);
      await this.$store.dispatch('customer/getMaintenance', this.$route.params.id);
    }
  }
</script>
