    ...
    <el-page-header @back="$router.go(-1)" title="" class="mb-3 py-3 px-3 shadow-md bg-white w-full rounded-lg">
        <template #content class="w-full">
            <div class="flex w-full justify-between items-center">
                ...
                <el-button @click="create()" size="mini" type="primary">Lưu</el-button>
            </div>
        </template>
    </el-page-header>
...
export default {
    ...
    methods: {
        create() {
            try {
                this.$store.dispatch('userRole/create', this.model)
                this.$message.success('Thêm quyền thành công')
                this.$router.push('/users/roles')
            }
            catch{
                this.$message.error('Thêm quyền thất bại')
            }
