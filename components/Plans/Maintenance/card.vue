<template>
  <div v-if="$attrs.value" class="w-full form-card">
    <span class="w-full flex mb-5">
      <div class="flex-grow">
        <h3 class="text-2xl font-bold"><i class='bx bx-menu'></i> Bảng giá Bảo trì Website</h3>
      </div>
      ...
      <div class="flex-grow">
        <div class="grid grid-cols-4 gap-4">
          <el-card v-for="(item, index) in $attrs.value.data" :key="index" class="box-card">
            ...
            <div class="text item">
              <p v-if="item.note">Ghi chú: <strong>{{ item.note }}</strong></p>
    ...
    >
      <el-form>
        ...
        <el-form-item label="Ghi chú">
          <el-input size="small" v-model="model.note" placeholder="Nhập ghi chú..."></el-input>
        </el-form-item>
      </el-form>
      <template>
    ...
    >
      <el-form>
        ...
        <el-form-item label="Ghi chú">
          <el-input size="small" v-model="selectedItem.note" placeholder="Nhập ghi chú..."></el-input>
        </el-form-item>
      </el-form>
      <template>
        <span class="dialog-footer">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async save() {
        try {
          await this.$store.dispatch('maintenancePlan/create', this.model);
          this.$message({
      ...
      async update() {
        try {
          await this.$store.dispatch('maintenancePlan/update', { id: this.selectedItem._id, data: this.selectedItem });
          this.$message({
      ...
      async remove(id) {
        try {
          this.$confirm('Bạn có muốn xóa dịch vụ này không?', 'Xóa dịch vụ', {
            ...
            await this.$store.dispatch('maintenancePlan/delete', id);
            this.$message({
                type: 'success',
