<template>
  <div v-if="$attrs.value" class="w-full form-card">
    <span class="w-full flex mb-5">
      <div class="flex-grow">
        <h3 class="text-2xl font-bold"><i class='bx bx-menu'></i> Bảng giá Hosting</h3>
      </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async save() {
        try {
          await this.$store.dispatch('hostingPlan/create', this.model);
          this.$message({
      ...
      async update() {
        try {
          await this.$store.dispatch('hostingPlan/update', { id: this.selectedItem._id, data: this.selectedItem });
          this.$message({
      ...
      async remove(id) {
        try {
          this.$confirm('Bạn có muốn xóa dịch vụ này không?', 'Xóa dịch vụ', {
            ...
            await this.$store.dispatch('hostingPlan/delete', id);
            this.$message({
                type: 'success',
