<template>
  <div v-if="$attrs.value" class="w-full form-card">
    <span class="w-full flex mb-5">
      <div class="flex-grow">
        <h3 class="text-2xl font-bold"><i class='bx bx-menu'></i> Bảng giá Nội dung marketing</h3>
      </div>
      ...
      <div class="flex-grow">
        <div class="grid grid-cols-4 gap-4">
          <el-card v-for="(item, index) in $attrs.value.data" :key="index" class="box-card">
            ...
            <div class="text item">
              <p>Số lượng bài viết: <strong>{{ item.number_of_articles }}</strong></p>
    ...
    >
      <el-form>
        ...
        <el-form-item label="Số lượng bài viết">
          <el-input size="small" v-model="model.number_of_articles" placeholder="Nhập số lượng bài viết..."></el-input>
        </el-form-item>
    ...
    >
      <el-form>
        ...
        <el-form-item label="Số lượng bài viết">
          <el-input size="small" v-model="selectedItem.number_of_articles" placeholder="Nhập số lượng bài viết..."></el-input>
        </el-form-item>
        <el-form-item label="Chi phí (*)">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async save() {
        try {
          await this.$store.dispatch('contentPlan/create', this.model);
          this.$message({
      ...
      async update() {
        try {
          await this.$store.dispatch('contentPlan/update', { id: this.selectedItem._id, data: this.selectedItem });
          this.$message({
      ...
      async remove(id) {
        try {
          this.$confirm('Bạn có muốn xóa dịch vụ này không?', 'Xóa dịch vụ', {
            ...
            await this.$store.dispatch('contentPlan/delete', id);
            this.$message({
                type: 'success',
