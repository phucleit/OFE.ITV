...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      search() {
        this.$store.dispatch('email/init', {search: this.searchStr});
      },

      handleCurrentChange(val) {
        this.$store.dispatch('email/init', {page: val});
      },

      getPeriodType() {
        this.$store.dispatch('email/getPeriodType');
      },

      getExpiredType() {
        this.$store.dispatch('email/getExpiredType');
      },
      ...
      async detail(id) {
        var data = await this.$store.dispatch("email/getDetail", id);
      ...
      async update() {
        try {
          await this.$store.dispatch('email/update', { id: this.selectedItem._id, data: this.selectedItem });
          this.$message({
      ...
      async remove(id) {
        try {
          this.$confirm('Bạn có muốn xóa dịch vụ này không?', 'Xóa dịch vụ', {
            ...
            await this.$store.dispatch('email/delete', id);
            this.$message({
                type: 'success',
    ...
    computed: {
      ...
      countAll() {
        return this.$store.state.email.countAll;
      },

      countPeriod() {
        return this.$store.state.email.countPeriod;
      },

      countExpired() {
        return this.$store.state.email.countExpired;
      }, 
    },
