...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      search() {
        this.$store.dispatch('ssl/init', {search: this.searchStr});
      },

      handleCurrentChange(val) {
        this.$store.dispatch('ssl/init', {page: val});
      },

      getPeriodType() {
        this.$store.dispatch('ssl/getPeriodType');
      },

      getExpiredType() {
        this.$store.dispatch('ssl/getExpiredType');
      },
      ...
      async detail(id) {
        var data = await this.$store.dispatch("ssl/getDetail", id);
      ...
      async update() {
        try {
          await this.$store.dispatch('ssl/update', { id: this.selectedItem._id, data: this.selectedItem });
          this.$message({
      ...
      async remove(id) {
        try {
          this.$confirm('Bạn có muốn xóa dịch vụ này không?', 'Xóa dịch vụ', {
            ...
            await this.$store.dispatch('ssl/delete', id);
            this.$message({
                type: 'success',
    ...
    computed: {
      ...
      countAll() {
        return this.$store.state.ssl.countAll;
      },

      countPeriod() {
        return this.$store.state.ssl.countPeriod;
      },

      countExpired() {
        return this.$store.state.ssl.countExpired;
      }, 
    },
