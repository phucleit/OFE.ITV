<template>
  <div v-if="$attrs.value" class="w-full plans-list">
    ...
    <el-table :data="$attrs.value.data" stripe style="width: 100%">
      ...
      <el-table-column label="Số lượng bài viết" width="170">
        <template slot-scope="scope">
          {{ scope.row.plan[0].number_of_articles }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="Ngày hết hạn" width="170">
        <template slot-scope="scope">
    ...
    >
      <el-form>
        <el-form-item label="Tên gói nội dung (*)">
  ...
  export default {
    ...
    methods: {
      async reload() {
        ...
        await this.$store.commit('content/initCountAll', (await this.$axios.get('/contents')).data);
        await this.$store.commit('content/initCountPeriod', (await this.$axios.get('/contents?periodType=1')).data);
        await this.$store.commit('content/initCountExpired', (await this.$axios.get('/contents?expiredType=2')).data);
      },

      search() {
        this.$store.dispatch('content/init', {search: this.searchStr});
      },

      handleCurrentChange(val) {
        this.$store.dispatch('content/init', {page: val});
      },

      getPeriodType() {
        this.$store.dispatch('content/getPeriodType');
      },

      getExpiredType() {
        this.$store.dispatch('content/getExpiredType');
      },
      ...
      async detail(id) {
        var data = await this.$store.dispatch("content/getDetail", id);
      ...
      async save() {
        try {
          await this.$store.dispatch('content/create', this.model);
          this.$message({
      ...
      async update() {
        try {
          await this.$store.dispatch('content/update', { id: this.selectedItem._id, data: this.selectedItem });
          this.$message({
      ...
      async remove(id) {
        try {
          this.$confirm('Bạn có muốn xóa dịch vụ này không?', 'Xóa dịch vụ', {
            ...
            await this.$store.dispatch('content/delete', id);
            this.$message({
                type: 'success',
    ...
    computed: {
      ...
      countAll() {
        return this.$store.state.content.countAll;
      },

      countPeriod() {
        return this.$store.state.content.countPeriod;
      },

      countExpired() {
        return this.$store.state.content.countExpired;
      }, 
    },
