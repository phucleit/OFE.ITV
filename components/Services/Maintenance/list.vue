...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      search() {
        this.$store.dispatch('maintenance/init', {search: this.searchStr});
      },

      handleCurrentChange(val) {
        this.$store.dispatch('maintenance/init', {page: val});
      },

      getPeriodType() {
        this.$store.dispatch('maintenance/getPeriodType');
      },

      getExpiredType() {
        this.$store.dispatch('maintenance/getExpiredType');
      },
      ...
      async detail(id) {
        var data = await this.$store.dispatch("maintenance/getDetail", id);
      ...
      async update() {
        try {
          await this.$store.dispatch('maintenance/update', { id: this.selectedItem._id, data: this.selectedItem });
          this.$message({
      ...
      async remove(id) {
        try {
          this.$confirm('Bạn có muốn xóa dịch vụ này không?', 'Xóa dịch vụ', {
            ...
            await this.$store.dispatch('maintenance/delete', id);
            this.$message({
                type: 'success',
    ...
    computed: {
      ...
      countAll() {
        return this.$store.state.maintenance.countAll;
      },

      countPeriod() {
        return this.$store.state.maintenance.countPeriod;
      },

      countExpired() {
        return this.$store.state.maintenance.countExpired;
      }, 
    },
