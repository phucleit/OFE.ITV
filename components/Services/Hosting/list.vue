<template>
  <div v-if="$attrs.value" class="w-full plans-list">
    ...
    <el-table :data="$attrs.value.data" stripe style="width: 100%">
      <el-table-column label="Tên gói" width="200">
      ...
      <el-table-column label="Chi phí" width="220">
        <template slot-scope="scope">
          {{ scope.row.plan[0].sell_price * 12 | currency(' đ', 0, { symbolOnLeft: false }) }} / năm
        </template>
      </el-table-column>
      ...
      <el-table-column label="Ngày hết hạn" width="200">
        <template slot-scope="scope">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      search() {
        this.$store.dispatch('hosting/init', {search: this.searchStr});
      },

      handleCurrentChange(val) {
        this.$store.dispatch('hosting/init', {page: val});
      },

      getPeriodType() {
        this.$store.dispatch('hosting/getPeriodType');
      },

      getExpiredType() {
        this.$store.dispatch('hosting/getExpiredType');
      },
      ...
      async detail(id) {
        var data = await this.$store.dispatch("hosting/getDetail", id);
      ...
      async update() {
        try {
          await this.$store.dispatch('hosting/update', { id: this.selectedItem._id, data: this.selectedItem });
          this.$message({
      ...
      async remove(id) {
        try {
          this.$confirm('Bạn có muốn xóa dịch vụ này không?', 'Xóa dịch vụ', {
            ...
            await this.$store.dispatch('hosting/delete', id);
            this.$message({
                type: 'success',
    ...
    computed: {
      ...
      countAll() {
        return this.$store.state.hosting.countAll;
      },

      countPeriod() {
        return this.$store.state.hosting.countPeriod;
      },

      countExpired() {
        return this.$store.state.hosting.countExpired;
      }, 
    },
